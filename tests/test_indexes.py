...
def index(name, _client: Client):
    idx_id = _client.indexes.create(name=name)
    try:
        yield idx_id
    finally:
        _client.indexes.delete(id=idx_id)
...
def test_delete_index(client: Client, vcr_cassette):
    idx_id = client.indexes.create(name="test_delete_index")
    client.indexes.delete(id=idx_id)
    assert client.indexes.get(id=idx_id) is None


def test_get_by_id(vcr_cassette, client: Client):
    with index("test_get_by_id", client) as idx_id:
        idx = client.indexes.get(id=idx_id)
        assert idx.id == idx_id
        assert idx.name == "test_get_by_id"


def test_get_by_name(vcr_cassette, client: Client):
    with index("test_get_by_name", client) as idx_id:
        idx = client.indexes.get(name="test_get_by_name")
        assert idx.id == idx_id
...
def test_list_indexes(vcr_cassette, client: Client):
    ...
    with index("test_list_indexes", client) as idx_id:
        idxs = client.indexes.list()
        assert len(idxs) == 1
        assert idxs[0].id == idx_id
        assert idxs[0].name == "test_list_indexes"

        with index("test_list_indexes_2", client) as idx_id_2:
            idxs = client.indexes.list()
            assert len(idxs) == 2
...
def test_index_document(vcr_cassette, client: Client):
    with index("test_index_document", client) as idx_id:
        doc_in = DocumentIn(content="Hello", metadata={"source": "test"})
        doc_out = client.indexes.index(id=idx_id, doc=doc_in)
...
def test_retrieve_document(vcr_cassette, client: Client):
    with index("test_retrieve_document", client) as idx_id:
        doc_in = DocumentIn(content="Hello", metadata={"source": "test"})
        client.indexes.index(id=idx_id, doc=doc_in)
        doc_retrieved = client.indexes.retrieve(id=idx_id, query="Hello", k=1)
...
def test_upload_file(client: Client, vcr_cassette):
    with index("test_upload_file", client) as idx_id:
        with open("/tmp/test.txt", "wb") as f:
            f.write(b"Hello")
            f.seek(0)
            client.indexes.upload_file(id=idx_id, file_path=f.name)
...
def test_retrieve_filters(client: Client, vcr_cassette):
    with index("test_retrieve_filters", client) as idx_id:
